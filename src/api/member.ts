...
export const readMemberRecord = async (userId: string) => {
...
export const updateMemberName = async ({name, userId}: {name: string, userId: string}) => {
    const { data, error } = await supabase.from("member").update({ name: name}).eq("user_id", userId).select("*");
    if(error){
        throw error
    }
    return data;
...
export const updateMemberLimit = async ({limit, userId}: {limit: number, userId: string}) => {
    const { data, error } = await supabase.from("member").update({ expense_limit: limit}).eq("user_id", userId).select("*");
    if(error){
        throw error
    }
    return data;
